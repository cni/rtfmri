...
class SessionFetcher(object):
    ...
    def choose_series(self):
        ...
        for i, series in enumerate(series_dirs, 1):
            ...
            if description != "Screen Save":
                print(" {:d}: {} ({:d} dicoms, {:d} volumes)".format(i, description, num_dicoms, n_volumes))
    ...
    def valid_subseries(self, src_paths):
        ...
        if left != 0:
            warnings.warn("Number of files is not an even multiple of the number of unique slice positions")

        print(("Using the first {:d} of {:d} prescribed volumes .").format(n_volumes, n_prescribed))
        return src_paths[:end-left]
    ...
    def build_nifti(self, series, nii_fname):
        ...
        if nii_fname is None: print("No output name given.")
